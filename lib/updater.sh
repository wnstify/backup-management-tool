...
get_release_url() {
  local version="$1"
  echo "${GITHUB_RELEASE_URL}/v${version}/backup-management-v${version}.tar.gz"
}
...
apply_update() {
  ...
  local source_dir
  source_dir=$(find "$extract_dir" -maxdepth 2 -name "backup-management.sh" -printf '%h\n' 2>/dev/null | head -1)

  if [[ -z "$source_dir" || ! -f "${source_dir}/backup-management.sh" ]]; then
  ...
  chmod +x "$SCRIPT_DIR/backup-management.sh"
...
do_update() {
  print_header
  echo "Update Backup Management Tool"
  echo "=============================="
  echo
  ...
  local temp_dir
  temp_dir=$(create_secure_temp "backup-mgmt-update")
...
check_for_updates_verbose() {
  ...
  if is_update_available "$VERSION" "$latest_version"; then
    ...
    echo
    echo "Run 'backup-management --update' or select Update from the menu."
  else
